...
function extractHistograms(canvas, options) {
  ...
  //Return value is the vector at each pixel 
  //with mag and orient for magnitude and orientation (in radians):
...
function getHistogram(elements, x, y, size, bins, canvas) {
  var histogram = zeros(bins);

  console.log("Y : ", y, "X : ", x);
  /*
  {
    console.log("hello" , (x + (size -1)));
    var ctx=canvas.getContext("2d");
    ctx.lineWidth="1";
    ctx.strokeStyle="green";
    // start-x, start-y, stop-x, stop-y
    ctx.rect(x ,y, (x + (size -1)),(y + size));
    // console.log('cordinat i histograms', i, j);
    // console.log("Y : ", i * cellSize, i * cellSize , "X : ", j * cellSize, j * cellSize);
    ctx.stroke();
  }*/
  ...
  for (var i = 0; i < size; i++) {
    for (var j = 0; j < size; j++) {
      ...
      // if(x < 2)
      //   console.log("elements[y + i]: ", y+i," [x + j]: ",x + j);
